...
import { searchPizza } from "../../redux/slices/searchPizzaSlice";
import debounce from "lodash.debounce";

function ContentCatalog({ title }) {
  ...
  const getData = async (searchValue) => {
    ...
    dispatch(searchPizza({ searchValue }));
  };
  const debounceChangeInput = debounce(getData, 500);
  const { items } = useSelector((state) => state.searchSlice);

  // loading
  const noSearchItems = (
    <p className="content__catalog-bottom-p">Такой пиццы у нас нет!</p>
  );
  const pizzas = items.map((obj) => (
    <Cart key={obj.title} {...obj} obj={obj} />
  ));
  return (
    <div className="content__catalog">
      <div className="content__catalog-top">
          {/* <div className="content__catalog-popupProduct"> */}
            {/* Продукт был добавлен в коризну */}
          {/* </div> */}
        <div className="content__catalog-top-title">{title}</div>
        <div className="content__catalog-top-find">
          <input
            ref={search}
            onChange={(e) => debounceChangeInput(e.target.value)}
            placeholder="Введите название пиццы..."
            type="text"
          />
        </div>
        <div className="content__catalog-top-filter">
          <div>
            <svg
              width="16"
              height="16"
